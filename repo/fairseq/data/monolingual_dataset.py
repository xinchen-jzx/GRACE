...
def collate(samples, pad_idx, eos_idx):
    ...
    return {
        ...
        'target': target,
        #'sent_index':[s['sent_index'] for s in samples] # my
    }
...
class MonolingualDataset(FairseqDataset):
    ...
    def __init__(self, dataset, sizes, src_vocab, tgt_vocab, add_eos_for_other_targets, shuffle,
                 targets=None, add_bos_token=False,args=None):
    ...
    def __getitem__(self, index):
        ...
        if self.args.gpt2_gen_mode:
            return {'id': index, 'source': source, 'target': past_target}
        else:
        ...
        Returns:
            dict: a mini-batch with the following keys:
                ...
                - `target` (LongTensor): a padded 2D Tensor of tokens in the
                  ...
                  on the right.
        """
        if "gpt2" in self.args.arch:
            return collate(samples, 50256, self.vocab.eos())
        else:
