...
class TokenBlockDataset(FairseqDataset):
    ...
    def __init__(
        ...
        self.gpt2_gen_mode = gpt2_gen_mode #wxh
    @property
    ...
    def __getitem__(self, index):
        ...
        if self.include_targets:
            ...
            if s == 0:
                if self.gpt2_gen_mode:
                    source = buffer
                    past_target = torch.cat(
                        [buffer[1: e], item.new([self.eos]) ])

                else:
                    source = torch.cat([item.new([self.eos]), buffer[0 : e - 1]]) #my
                    past_target = torch.cat(
