...
class SequenceScorer(object):
    ...
    def generate(self, models, sample, **kwargs):
        ...
        # my
        def combine_knn_and_vocab_and_condition_probs(prob, condition):
            pass
        ...
        for model in models:
            model.eval()
            decoder_out = model(**net_input,args=self.args)
            attn = decoder_out[1]
            ...
            if 'knn_dstore' in kwargs:
                ...
                probs = combine_knn_and_vocab_probs(
                            yhat_knn_prob, probs, self.args.lmbda) # yhat_knn_prob & probs: sent-num, sent-length
            condition = kwargs.get("condition", False)
            if condition:
                probs = combine_knn_and_vocab_and_condition_probs(
                    probs, self.args.alpha)
        ...
        for i in range(bsz):
            ...
            hypos.append([{
                ...
                'dstore_keys': decoder_out[1]['last_ffn_input'][start_idxs[i]:,i,:] if self.args.save_knnlm_dstore else None,
            }])
        return hypos
