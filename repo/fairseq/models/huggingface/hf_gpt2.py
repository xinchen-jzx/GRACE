...
class HuggingFaceGPT2LanguageModel(FairseqLanguageModel):
    ...
    def add_args(parser):
        ...
        # fmt: off
        parser.add_argument('--pre-trained-path', type=str, metavar='N',
        ...
        parser.add_argument('--num-layers', type=int, metavar='N',
                            help='num layers')
        parser.add_argument('--return-all', action='store_true', required=False,
                            help='choose to train from pretrained BERT ')
...
class HuggingFaceGPT2Decoder(FairseqIncrementalDecoder):
    def __init__(self, args, task):
        ...
        self.model = GPT2LMHeadModel.from_pretrained(args.pre_trained_path)
    ...
    def forward(
        ...
        # torch.save(self.model.state_dict(), r"G:\Projects\KNNLM\models\dstore\sentiment\gpt2-medium/pytorch_model.bin")
        # exit()
        if not getattr(args, 'gpt2_gen_mode', False):
            self.gpt2_gen_mode = False
        else:
            self.gpt2_gen_mode = True
        features = self.extract_features(prev_output_tokens, incremental_state)


        if self.return_all:
            lm_logits = self.model.lm_head(features[0])
            return (lm_logits, {'attn' : [None],'last_ffn_input' : torch.transpose(features[0], 0, 1)})
        else:
    ...
    def extract_features(
        ...
        if incremental_state:
            ...
            self.set_incremental_state(incremental_state, "past_key_values", outputs[1])
        if self.return_all:
            return outputs
...
def hf_gpt2_medium(args):
    ...
    args.decoder_embed_dim = getattr(args, "decoder_embed_dim", 1024) # my
