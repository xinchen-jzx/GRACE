...
from fairseq.data import LMCTGDataset
from fairseq.meters import StopwatchMeter, TimeMeter
from fairseq.sequence_scorer import SequenceScorer
from fairseq.knnlm import KNN_Dstore

from fairseq.models.huggingface.hf_gpt2 import HuggingFaceGPT2LanguageModel
    ...
    with progress_bar.build_progress_bar(args, itr) as t:
        ...
        if args.save_knnlm_dstore:
            ...
            if args.dstore_fp16:
                ...
                my
                '''
                dstore_sent_indexs = np.memmap(args.dstore_mmap+'_sent_indexs.npy', dtype=np.int16, mode='w+', shape=(args.dstore_size, 1))
            else:
                print('Saving fp32')
                ...
                my
                '''
                dstore_sent_indexs = np.memmap(args.dstore_mmap+'_sent_indexs.npy', dtype=np.int, mode='w+', shape=(args.dstore_size, 1))
        ...
        for ex_i, sample in enumerate(t):
            ...
            gen_timer.start()
            num_of_words += sample['net_input']['src_lengths'].sum(-1)
            if args.knnlm:
                ...
                if args.save_knnlm_dstore:
                    ...
                    if args.dstore_fp16:
                        ...
                        dstore_keys[dstore_idx:shape[0]+dstore_idx] = hypo['dstore_keys'][:shape[0],:].view(
                            shape[0], args.decoder_embed_dim).cpu().numpy().astype(np.float32)
                        dstore_vals[dstore_idx:shape[0]+dstore_idx] = hypo['tokens'].view(
                            -1, 1).cpu().numpy().astype(np.int)
                        #dstore_sent_indexs[dstore_idx:shape[0]+dstore_idx] = np.full((hypo['tokens'].shape[0],1),sample['sent_index'][i])
                        dstore_sent_indexs[dstore_idx:shape[0]+dstore_idx] = np.full((hypo['tokens'].shape[0],1),sample['id'][i])
...
def cli_main(ctg):
    if ctg:
        parser = options.get_eval_ctg_lm_parser(default_task="ctg_language_modeling")
    else:
